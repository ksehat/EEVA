...
class MyGeneticAlgorithm():
    ...
    def __init__(self, config: dict, function, symbol, data_step, n_crossover: int = None,
    ...
    def evaluate(self, member):
        ...
        output = self.function(member, self.symbol, self.data_step)
        if not output:
            if self.maximize:
                return 1e-5
            else:
    ...
    def run(self, keep_frac: float = 3, crossover_frac: float = 3, mutate_frac: float = 3):
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=14) as executor:
            for member, score in zip(member_list, executor.map(self.evaluate, member_list)):
        ...
        while run_iter < self.run_iter:
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=14) as executor:
                for member, score in zip(new_population_score_df['members'][n_keep:],
                                         executor.map(self.evaluate, new_population_score_df['members'][n_keep:])):
                    score_list.append(score)
            new_population_score_df['score'][n_keep:] = score_list
...
#
#
# ga = MyGeneticAlgorithm(config=ali, function=f, run_iter=5, population_size=250, n_crossover=3,
#                         crossover_mode='random', symbol='ETHUSDT', data_step='30m')
# def main():
#     best_params = ga.run(keep_frac = 20, crossover_frac = 10, mutate_frac = 10)
#     print(best_params)
#
