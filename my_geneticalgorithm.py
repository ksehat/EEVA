...
class MyGeneticAlgorithm():
    ...
    def run(self, keep_frac: float=10, crossover_frac: float=10, mutate_frac: float=35):
        ...
        new_population_score_df = pd.DataFrame({'members': [list(range(3))]*self.population_size,
        ...
        while run_iter < self.run_iter:
            ...
            while iter_cross < n_crossover:
                first_parent  = random.choices(population_score_df['members'], weights=population_score_df['score'])
                second_parent = random.choices(population_score_df['members'], weights=population_score_df['score'])
                ...
                if child not in list(new_population_score_df['members'][:n_keep+iter_cross]):
                    new_population_score_df.at[n_keep+iter_cross, 'members'] = copy.deepcopy(child)
                    new_population_score_df.at[n_keep+iter_cross, 'score'] = copy.deepcopy(self.evaluate(child))
                    iter_cross+=1
            # endregion
            # region mutation
            iter_mut = 0
            while iter_mut < n_mutate:
                mut_member = copy.deepcopy(random.choices(population_score_df['members'], weights=population_score_df['score'])[0])
                child = copy.deepcopy(self.mutate(mut_member))
                if child == None: continue
                if child not in list(new_population_score_df['members'][:n_keep+n_crossover+iter_mut]):
                    new_population_score_df.at[n_keep+n_crossover+iter_mut, 'members'] = copy.deepcopy(child)
                    new_population_score_df.at[n_keep+n_crossover+iter_mut, 'score'] = copy.deepcopy(self.evaluate(child))
                    iter_mut+=1
            # endregion
            # region random
            iter_rand = 0
            while iter_rand < n_random:
                ...
                if random_member not in list(new_population_score_df['members'][:n_keep+n_crossover+n_mutate+iter_rand]):
                    new_population_score_df.at[n_keep+n_crossover+n_mutate+iter_rand, 'members'] = copy.deepcopy(random_member)
