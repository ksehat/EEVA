...
def xab_enter_check(df, date_pointer, xab, enter, enter_price, wait_candles):
    if xab[2]:
        if df['close'][date_pointer] >= xab[0][2]:
            if df['high'][date_pointer] <= xab[0][0] + abs(xab[0][2] - xab[0][3]):
                enter = 1
                enter_price = df['close'][date_pointer]
            else:
                return enter, enter_price, 1
        if enter:
            xab[3] = xab[0][0] + abs(xab[0][2] - xab[0][3])
            xab[4] = xab[0][0] + abs(xab[0][2] - xab[0][3])
            if df['MACD1_Hist'][date_pointer] > 0:
                ...
                while df['MACD1_Hist'][date_pointer - i] > 0:
                    if df['high'][date_pointer - i] > xab[4] and df['high'][date_pointer - i] <= \
                            xab[0][0] + abs(xab[0][2] - xab[0][3]):
                        xab[4] = df['high'][date_pointer - i]
                    i += 1
    if not xab[2]:
        if df['close'][date_pointer] <= xab[0][2]:
            if df['low'][date_pointer] >= xab[0][0] - abs(xab[0][2] - xab[0][3]):
                enter = 1
                enter_price = df['close'][date_pointer]
            else:
                return enter, enter_price, 1
...
def xab_reject_decision(df, dp, xab, wait_candles):
    if xab[2] == 1:
        if df['low'][dp] > xab[0][0]:
            return 1
    if xab[2] == 0:
        if df['high'][dp] < xab[0][0]:
            return 1
        ...
        for date_pointer in range(xab[1][4], len(df)):
            ...
            if enter == 1:  # If it is in trade
                ...
                if date_pointer > len(df) - 1:
                    break
                if xab[2] == 1:
                    if macd_phase_change(df, date_pointer) and df['MACD1_Hist'][
                        date_pointer] < 0:
                        xab[3] = xab[4]
                        if xab[4] != xab[0][0]:
                            sl_at_this_candle = 1
...
def main():
    ...
    if run_mode == 1:
        ...
        plot_height = 1000
        macd_list = [
            [5, 3, 3, symbol, start_date, data_step, end_date, leverage]
        ]
