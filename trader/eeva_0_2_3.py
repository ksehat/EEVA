...
def macd_phase_change(df, date_pointer):
    if df['MACD2_Hist'][date_pointer] * df['MACD2_Hist'][date_pointer - 1] < 0:
        return True
    else:
        return False
...
def xab_enter_check(df, date_pointer, xab, enter, enter_price, wait_candles):
    # here we are using xab[6] for the exceptional states when high of the price touches B and in
    # the next candle high is not higher than B
    if xab[2] and (df['high'][date_pointer] >= xab[0][2] or xab[6]):
        ...
        if enter:
            ...
            if df['MACD2_Hist'][date_pointer] < 0:
                xab[4] = df['low'][date_pointer]  # C is placed in sudo_sl
                i = 0
                while df['MACD2_Hist'][date_pointer - i] < 0:
...
def stop_loss_trail(df, date_pointer, xab):
    if xab[2] == 1:
        ...
        if df['MACD2_Hist'][date_pointer] > 0: xab[3] = xab[4]
    if xab[2] == 0:
        if df['MACD2_Hist'][date_pointer] > 0:
...
def trader(args):
    ...
    df = DataHunter(symbol=symbol, start_date=start_date, end_date=end_date,
                    step=data_step).prepare_data(macd_slow=args[0], macd_fast=args[1],
                                                 macd_sign=args[2], macd2_slow=10, macd2_fast=9,
                                                 macd2_sign=8)
    ...
    for xab in XAB_list:
        ...
        # xabc = [[X, A, B, C], [index_X, index_A, index_B, index_C, index_4], xab_flag, sl,
        # sudo_sl, dont_find_C, candle_counter, C_at_this_candle]
            if enter == 0:
...
def main():
    ...
    if run_mode == 1:
        ...
        plot_height = 1000
        macd_list = [
            [30, 24, 12, symbol, start_date, data_step, end_date, leverage]
        ]
