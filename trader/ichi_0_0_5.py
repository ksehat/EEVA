    ...
    for dp in range(len(df)):
        ...
        if enter == 1:
            enter, exit_price, limit_exit = exit_check(df, dp, ls, enter, enter_price,
                                                       exit_price, sl_percent, limit_percent)
            if enter == 0:
                index_sell = dp
                ...
                if (ls == 0 and max(df['high'][index_buy + 1:index_sell + 1]) >= (
                    ...
                    if limit_exit == 1:
                        ...
                        money = (1 + pl) * money
                    if limit_exit == 0:
                        if (ls == 0 and df['high'][dp] >= (1 + limit_percent) * enter_price) or \
                                (ls == 1 and df['low'][dp] <= (1 - limit_percent) * enter_price):
                            pl = -sl_percent - trade_fee
                            pl_list.append(pl)
                            money = (1 + pl) * money
                        else:
                            money, pl = money_calc(df, money, enter_price, index_buy, index_sell,
                                                   ls,
                                                   trade_fee, pl_list)
                else:
                    pl = -sl_percent - trade_fee
