...
def xab_enter_check(df, date_pointer, xab, enter, enter_price):
    if xab[2] and df['high'][date_pointer] >= xab[0][2]:
        if date_pointer == xab[1][3]:
            ...
            xab[3] = xab[0][3]  # C is placed in sl
            xab[4] = xab[0][3]  # C is placed in sudo_sl
        if enter == 1 and df['MACD1_Hist'][date_pointer] < 0:
            i = 0
...
if run_mode == 1:
    ...
    os.chdir('D:/Python projects/EEVA/trader/Genetic Results/Sys1.2.0/Genetic/weighted money')
