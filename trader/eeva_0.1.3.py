...
def macd_phase_change(df, date_pointer):
    if df['MACD_Hist'][date_pointer] * df['MACD_Hist'][date_pointer - 1] < 0:
        return True
    else:
        return False
        ...
        for data_step in data_steps:
            df = DataHunter(symbol=symbol, start_date=start_date, end_date=end_date,
                            step=data_step).prepare_data(macd_slow=args[0], macd_fast=args[1],
                                                         macd_sign=args[2])
            df['MACD_Hist'] = df['MACD1_Hist']
            df['MACD_ZC'] = df['MACD1_ZC']
            ...
            Profit_Loss_Table_by_Year_Month_for_symbol = \
                pd.concat(
                    ...
                    axis=1)
        Profit_Loss_Table_by_Year_Month_for_symbol.to_csv(
            f'{symbol}-{start_date}-{data_steps}-{args}-{money}.csv',
            index=True)
