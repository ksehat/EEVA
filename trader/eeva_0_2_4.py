...
from utils.utils import create_pl_table, max_num_of_loss_trades
        ...
        for date_pointer in range(xab[1][4], len(df)):
            if enter == 0:
                ...
                if enter == 1:
                    ...
                    if (xab[2] and df['low'][date_pointer] < xab[0][3]) or (not xab[2] and df[
                        'high'][date_pointer] > xab[0][3]):
                        enter = 0
                    ...
                    if xab[2] == 1:
                        if df['low'][date_pointer] < xab[0][3]:
                            ...
                            money = extra_calculations(df, df2, xab, money, enter_price,
                                                       ...
                                                       print_outputs, trade_fee, profit_loss_list,0)
                            break
                        if df['high'][date_pointer] >= xab[0][2] + abs(xab[0][2]-xab[0][3]):
                            enter = 0
                            index_sell = date_pointer
                            exit_price = xab[0][2] + abs(xab[0][2]-xab[0][3])
                            date_of_trade_list.append(df['timestamp'][date_pointer])
                            money = extra_calculations(df, df2, xab, money, enter_price,
                                                       exit_price, index_buy, index_sell, leverage,
                    ...
                    if xab[2] == 0:
                        if df['high'][date_pointer] > xab[0][3]:
                            ...
                            money = extra_calculations(df, df2, xab, money, enter_price,
                                                       ...
                                                       profit_loss_list, 0)
                            break
                        if df['low'][date_pointer] <= xab[0][2] - abs(xab[0][2]-xab[0][3]):
                            enter = 0
                            index_sell = date_pointer
                            exit_price = xab[0][2] - abs(xab[0][2]-xab[0][3])
                            date_of_trade_list.append(df['timestamp'][date_pointer])
...
def main():
    ...
    if run_mode == 1:
        ...
        plot_height = 1000
        macd_list = [
            [40, 7, 10, symbol, start_date, data_step, end_date, leverage]
        ]
