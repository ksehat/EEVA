        ...
        for date_pointer in range(xab[1][4], len(df)):
            ...
            if enter == 1:
                if date_pointer > len(df) - 1:
                    break
                if xab[2] == 1:
                    loss_trade = abs(xab[0][2] - xab[0][3])/xab[0][2]
                    profit_trade = rr * loss_trade + (rr + 1) * trade_fee
                    target_price = xab[0][2] * (profit_trade + 1)
                    if df['low'][date_pointer] < xab[0][3]:
                        enter = 0
                        index_sell = date_pointer
                        exit_price = xab[0][3]
                        ...
                        date_of_trade_list.append(df['timestamp'][date_pointer])
                        break
                    if df['high'][date_pointer] > target_price:
                        enter = 0
                        index_sell = date_pointer
                        ...
                        money = money + profit_const * money
...
def main():
    ...
    if run_mode == 1:
        ...
        plot_height = 1000
        macd_list = [
            [52, 48, 18, symbol, start_date, data_step, end_date, leverage]
        ]
