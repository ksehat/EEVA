...
def stop_loss_trail(df, date_pointer, xab):
    if xab[2] == 1:
        if df['MACD1_Hist'][date_pointer] < 0:
            if macd_phase_change(df, date_pointer) or df['low'][date_pointer] - (xab[
                0][2] - xab[0][3]) <= xab[4]:
                xab[4] = df['low'][date_pointer] - (xab[0][2] - xab[0][3])
        # if df['MACD1_Hist'][date_pointer] > 0:
        #     if xab[4] >= xab[0][3]:
        #         xab[3] = xab[4]
    if xab[2] == 0:
        if df['MACD1_Hist'][date_pointer] > 0:
            if macd_phase_change(df, date_pointer) or df['high'][date_pointer] - (xab[
                0][2] - xab[0][3]) >= xab[4]:
                xab[4] = df['high'][date_pointer] - (xab[0][2] - xab[0][3])
        # if df['MACD1_Hist'][date_pointer] < 0:
        #     if xab[4] <= xab[0][3]:
        #         xab[3] = xab[4]
        ...
        for date_pointer in range(xab[1][4], len(df)):
                if enter == 0:
                    ...
                    if xab[0][3]:
                        ...
                        xab[3] = xab[0][3] - (xab[0][2] - xab[0][3])
                        xab[4] = xab[0][3] - (xab[0][2] - xab[0][3])
                ...
                if enter == 1:
                    ...
                    if xab[2] == 1:
                        if macd_phase_change(df, date_pointer) and df['MACD1_Hist'][
                            ...
                            if xab[4] >= xab[3]:
                                xab[3] = xab[4]
                                if xab[4] != xab[0][3] - (xab[0][2] - xab[0][3]):
                                    sl_at_this_candle = 1
