...
def exit_check(df, dp, ls, enter, enter_price, exit_price):
    if ls == 0:
        if df['low'][dp] <= (1 - 0.01) * enter_price:
            exit_price = (1 - 0.01) * enter_price
            return 0, exit_price
        if df['high'][dp] >= (1 + 0.010) * enter_price:
            exit_price = (1 + 0.010) * enter_price
            return 0, exit_price
    ...
    if ls == 1:
        if df['high'][dp] >= (1 + 0.01) * enter_price:
            exit_price = (1 + 0.01) * enter_price
            return 0, exit_price
        if df['low'][dp] <= (1 - 0.010) * enter_price:
            exit_price = (1 - 0.010) * enter_price
            return 0, exit_price
    ...
    for dp in range(len(df)):
        if enter == 0:
            ...
            if enter == 1:
                ...
                continue
        if enter == 1:
            enter, exit_price = exit_check(df, dp, ls, enter, enter_price, exit_price)
            if enter == 0:
                index_sell = dp
