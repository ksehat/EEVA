    ...
    for xab in XAB_list:
        ...
        for item in xab:
            if isinstance(item, list):
                item_temp.extend(item)
            else:
                item_temp.append(item)
        XAB_list_final.append(item_temp)
    XAB_df = pd.DataFrame(XAB_list_final)
    df = XAB_df.filter([0, 1, 2, 3, 4, 5, 6, 7, 9, 15])
    df.dropna(axis=0, inplace=True)
    df.reset_index(inplace=True, drop=True)
    df2 = df[df.iloc[:, -1] != 0]
    df2['class'] = 0
    df2.loc[df2[15] > 0, 'class'] = 1
    df2.loc[df2[15] < 0, 'class'] = 0
...
def main():
    ...
    if run_mode == 1:
        ...
        plot_height = 1000
        macd_list = [
            [26, 24, 14, symbol, start_date, data_step, end_date, leverage]
        ]
        for macd_value in macd_list:
            XAB_df = trader(macd_value)
            XAB_df.to_csv(f'Classified_XAB_{macd_value[:-2]}.csv')
