...
def xab_enter_check(df, df2, date_pointer, date_pointer2, xab, enter, virtual_enter, fibo1):
    if xab[2] and df['close'][date_pointer] >= xab[0][2]:
        if df['high'][date_pointer] < (fibo1 * (abs(xab[0][2] - xab[0][3])) + xab[0][3]):
            enter = 1
    ...
    if not xab[2] and df['close'][date_pointer] <= xab[0][2]:
        if df['low'][date_pointer] > (-fibo1 * (abs(xab[0][2] - xab[0][3])) + xab[0][3]):
            enter = 1
            xab[3] = xab[0][3]  # C is placed in sl
            xab[4] = xab[0][3]  # C is placed in sudo_sl
            if df['MACD1_Hist'][date_pointer] > 0:
...
def enter_check_with_cheklist(df, dp, enter, enter_with_checklist, XAB_check_list1, money, money1,
    ...
    if XAB_check_list:
        ...
        if (xab_temp[2] == 1 and df['close'][dp] > xab_temp[0][2]) or (xab_temp[2] == 0 and df[
            ...
            if (xab_temp[2] == 1 and df['high'][dp] < (
                    fibo1 * (abs(xab_temp[0][2] - xab_temp[0][3])) + xab_temp[0][3])) \
                    or (xab_temp[2] == 0 and df['low'][dp] > (
                    -fibo1 * (abs(xab_temp[0][2] - xab_temp[0][3])) + xab_temp[0][3])):
                enter = 1
                    ...
                    if xab == xab_buy:
                        for date_pointer2 in [date_pointer22, date_pointer22 + 1]:
                            ...
                            if xab[2] == 1:
                                ...
                                elif df2['high'][date_pointer2] > (fibo1 * abs(xab[0][2] - xab[0][
                                    3]) + xab[0][3]):
                                    if flag1 == 0:
                                        ...
                                        # print('money1:', money1)
                                        flag1 = 1
                                    if df2['high'][date_pointer2] > (fibo2 * abs(xab[0][2] - xab[0][
                                        3]) + xab[0][3]):
                                        enter = 0
                                        enter_with_checklist = 0
                                        xab_buy = None
                                        flag1 = 0
                                        index_sell = date_pointer2
                                        exit_price = (fibo2 * abs(xab[0][2] - xab[0][3]) + xab[
                                            0][3])
                                        # print_trade(df, df2, X, A, B, xab, enter_price, exit_price,
                                        #             index_X, index_A, index_B,
                                        #             index_buy, index_sell)
...
for symbol, data_step in coins_datastep_list:
    best_params = GA.run()
    print(best_params)
    best_params.to_csv(f'Genetic-variance-v2.0.3-{symbol}-{start_date}-{data_step}.csv',
                       index=True)